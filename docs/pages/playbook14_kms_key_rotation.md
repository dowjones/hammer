---
title: KMS key rotation issues
keywords: playbook14
sidebar: mydoc_sidebar
permalink: playbook14_kms_key_rotation.html
---

# Playbook 14: KMS key rotation issues

## Introduction

This playbook describes how to configure Dow Jones Hammer to detect kms key rotation issues.

## 1. Issue Identification

Dow Jones Hammer identifies those KMS keys' ```CreateDate``` parameters. In case the difference between this parameter's value for a key and the current date exceeds the threshold set in the Dow Jones Hammer configuration.
...
|Designation   |Path                  |
|--------------|:--------------------:|
|Initialization|`hammer/identification/lambdas/kms-keyrotation-issues-identification/initiate_to_desc_kms_key_rotation.py`|
|Identification|`hammer/identification/lambdas/kms-keyrotation-issues-identification/describe_kms_key_rotation.py`        |
...
Additionally Dow Jones Hammer tries to detect person to report issue to by examining KMS keys created time and present time. In case the difference this parameter's value for a key and current date exceeds threshold **valid JIRA/Slack user**:
...
```
hammer/reporting-remediation/reporting/create_kms_key_rotation_issue_tickets.py
```
...
To identify and report issues of this type, you should add the following parameters in the **kms_keys_rotation** section of the **config.json** file:

|Parameter Name                |Description                            | Default Value|
|------------------------------|---------------------------------------|:------------:|
|`enabled`                     |Toggles issue detection for this issue |`true`|
|`ddb.table_name`              |Name of the DynamoDB table where Dow Jones Hammer will store the identified issues of this type| `hammer-kms-key-rotation` |
...
You can define exceptions to the general automatic remediation settings for specific KMS keys. To configure such exceptions, you should edit the **kms-keys-rotation** section of the **whitelist.json** configuration file as follows:

|Parameter Key | Parameter Value(s)|
|:------------:|:-----------------:|
|AWS Account ID|KMS key ids(s)|

Sample **whitelist.json** section:
```
"kms_keys_rotation": {
    "123456789012": ["kms_key1", "kms_key2"]
}
```
...
|---------------|--------------------------------------------|
|Initialization |`/aws/lambda/initiate-kms-key-rotation`|
|Identification |`/aws/lambda/describe-kms-key-rotation`|
...
|-----------|---------------------------------------------------------|
|Reporting  |`reporting.create_kms_keyrotation_issue_tickets`|
...
|-------------|:----:|----------------------------------|------------------------------------------------|
|`id`       |string|kms_key id                 |`test-key-id`                            |
|`name`     |string|KMS key arn name       |`key_arn`                                         |
|`tags`       |map   |Tags associated with KMS key |`{"Name": "TestKey", "service": "archive"}`|