...
resource "aws_s3_bucket_object" "sqs-unencrypted-queues-identification" {
    bucket = "${var.s3bucket}"
    key    = "lambda/${format("sqs-unencrypted-queues-identification-%s.zip", "${md5(file("${path.module}/../../../packages/sqs-unencrypted-queues-identification.zip"))}")}"
    source = "${path.module}/../../../packages/sqs-unencrypted-queues-identification.zip"
}
