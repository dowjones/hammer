...
def automation_cronjob(config):
    ...
    modules = [
        ("S3 ACL Public Access",      config.s3acl,               "create_s3bucket_acl_issue_tickets",    
         "clean_s3bucket_acl_permissions"),
        ("Insecure Services",         config.sg,                  "create_security_groups_tickets",       
         "clean_security_groups"),
        ("IAM User Inactive Keys",    config.iamUserInactiveKeys, "create_iam_key_inactive_tickets",      
         "clean_iam_keys_inactive"),
        ("IAM User Key Rotation",     config.iamUserKeysRotation, "create_iam_key_rotation_tickets",      
         "clean_iam_key_rotation"),
        ("S3 Policy Public Access",   config.s3policy,            "create_s3bucket_policy_issue_tickets", 
        ...
        ("EC2 Public Images",         config.publicAMIs,          "create_public_ami_issue_tickets",      
         "clean_ami_public_access"),
        ("SQS Public Access",         config.sqspolicy,           "create_sqs_policy_issue_tickets",          
         "clean_sqs_policy_permissions"),
        ("S3 Unencrypted Buckets",    config.s3Encrypt,           "create_s3_unencrypted_bucket_issue_tickets", 
         "clean_s3bucket_unencrypted"),
        ...
        ("ECS Privileged Access Issues", config.ecs_privileged_access, "create_ecs_privileged_access_issue_tickets", 
         None),
        ("ECS Logging Issues", config.ecs_logging, "create_ecs_logging_issue_tickets", None),
        ("ECS External Image Sources", config.ecs_external_image_source, 
        ...
        ("Elasticsearch Public Access", config.esPublicAccess, "create_elasticsearch_public_access_issue_tickets",
         "clean_elasticsearch_policy_permissions")
    ]
