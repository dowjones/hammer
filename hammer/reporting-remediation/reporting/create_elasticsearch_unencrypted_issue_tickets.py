...
class CreateElasticSearchUnencryptedDomainTickets(object):
    ...
    def create_tickets_elasticsearch_unencryption(self):
        ...
        jira = JiraReporting(self.config, module='esUnencryptedDomain')
            ...
            for issue in issues:
                ...
                if issue.timestamps.reported is not None:
                    ...
                    if (in_temp_whitelist or issue.status in [IssueStatus.Tempwhitelist]) and issue.timestamps.temp_whitelisted is None:
                        ...
                        logging.debug(f"Closing {issue.status.value} Elasticsearch unencrypted domain "
                                      f"'{domain_name}' issue")

                        comment = (f"Closing {issue.status.value} Elasticsearch unencrypted domain "
                                   f"'{domain_name}' issue in '{account_name} / {account_id}' account, '{region}' "
                                   f"region")
