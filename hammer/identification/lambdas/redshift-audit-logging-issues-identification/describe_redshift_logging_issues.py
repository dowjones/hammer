...
def lambda_handler(event, context):
    """ Lambda handler to evaluate Redshift logging enabled or not. """
    ...
    try:
        ...
        ddb_table = main_account.resource("dynamodb").Table(config.redshift_logging.ddb_table_name)
        ...
        logging.debug(f"Checking logging enabled or not for Redshift clusters in {account}")

        # existing open issues for account to check if resolved
        open_issues = IssueOperations.get_account_open_issues(ddb_table, account_id, RedshiftLoggingIssue)
