...
def lambda_handler(event, context):
    """ Lambda handler to evaluate Elasticsearch publicly accessible domains """
    ...
    try:
        ...
        ddb_table = main_account.resource("dynamodb").Table(config.esPublicAccess.ddb_table_name)
        ...
        logging.debug(f"Checking for Elasticsearch publicly accessible domains in {account}")

        # existing open issues for account to check if resolved
        open_issues = IssueOperations.get_account_open_issues(ddb_table, account_id, ESPublicAccessIssue)
        ...
        logging.debug(f"Elasticsearch publicly accessible domains in DDB:\n{open_issues.keys()}")

        checker = ESDomainChecker(account=account)
        if checker.check():
            for domain in checker.domains:
                if domain.public:
                    issue = ESPublicAccessIssue(account_id, domain.name)
                    ...
                    if config.esPublicAccess.in_quarantine_list(account_id, domain.name):
        ...
        logging.exception(f"Failed to check Elasticsearch publicly accessible domains "
    ...
    if len(payload['regions']) > 0:
        try:
            ...
            logging.exception("Failed to chain Elasticsearch publicly accessible domains checking")

    logging.debug(f"Checked Elasticsearch publicly accessible domains in '{region}' for '{account_id} ({account_name})'")
